...
// This file is a derivative of the file astcenccli_error_metrics.cpp,
// with the following modifications:
// - modified imports
// - made compute_error_metrics return a result struct instead of printing
// - format code with default VSCode C/C++ formatter
	...
	int fstop_hi)
{
	...
	astcenc_error_metrics result;
	// set result values to -1 to indicate that the value is not computed
	result.psnr = -1;
	result.psnr_rgb = -1;
	result.psnr_alpha = -1;
	result.peak_rgb = -1;
	result.mspnr_rgb = -1;
	result.log_rmse_rgb = -1;
	result.mean_angular_errorsum = -1;
	result.worst_angular_errorsum = -1;
	...
	if (componentmask & 8)
	{
		...
		result.psnr_rgb = rgb_psnr;
	}
	else
	{
		// printf("    PSNR (LDR-RGB):           %9.4f dB\n", psnr);
		result.psnr_rgb = psnr;
	}
	...
	if (compute_normal_metrics)
	{
		...
		result.mean_angular_errorsum = mean_angular_errorsum;
		result.worst_angular_errorsum = worst_angular_errorsum;
	}
